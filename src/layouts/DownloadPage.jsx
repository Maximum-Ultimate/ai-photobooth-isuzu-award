...
export default function DownloadPage() {
  ...
  const themeBtn = () =>
    isIPA()
      ...
      : "bg-purple-500 hover:bg-purple-400 shadow-[0_15px_30px_rgba(234,179,8,0.3)]";
  const themeGlow = () =>
    isIPA()
      ? "shadow-[0_0_50px_rgba(37,99,235,0.2)]"
      : "shadow-[0_0_50px_rgba(234,179,8,0.2)]";
  ...
  return (
    ...
    >
      {/* Dark Overlay biar konten nongol */}
      ...
      {/* Header */}
      <div
        class={`relative z-10 w-full max-w-md flex flex-col items-center gap-1 border-b-2 ${themeBorder()} pb-6 mb-10 pt-8`}
      >
      ...
      {/* Photo Preview */}
      <div
        class={`relative z-10 w-full max-w-md aspect-[3/4] bg-zinc-900 rounded-[40px] overflow-hidden border-2 border-white/10 ${themeGlow()} mb-8 animate-pop`}
      >
        <Show
          when={photoUrl()}
          fallback={
      ...
      {/* Tip */}
      <div class="relative z-10 mb-8 bg-white/5 border border-white/10 py-4 px-8 rounded-2xl flex items-center gap-4 hint-pulse">
        <div
      ...
      <div class="relative z-10 w-full max-w-md flex flex-col gap-6">
        <button
          ...
          disabled={!photoUrl()}
          class={`w-full py-7 rounded-[30px] flex items-center justify-center gap-5 font-black uppercase text-2xl transition-all active:scale-95 text-white disabled:bg-zinc-800 disabled:text-zinc-600 ${themeBtn()}`}
        >
          <DownloadIcon size={32} />
          Get Photo
        </button>

        <div class="flex flex-col items-center gap-2 mt-6 text-zinc-500">
